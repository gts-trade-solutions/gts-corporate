 ...
 *
 * The cover leads the card; on the featured card it takes the left half at lg,
 * so the lead post reads as a feature rather than a wider copy of the others.
 */
export function PostCard({ post, featured = false }: { post: BlogPost; featured?: boolean }) {
  return (
    ...
    >
      <div
        className={`relative shrink-0 overflow-hidden bg-navy-900 ${
          featured ? "aspect-[16/9] lg:aspect-auto lg:min-h-[340px]" : "aspect-[2/1] sm:aspect-[16/9]"
        }`}
      >
        <Media
          slot={post.cover}
