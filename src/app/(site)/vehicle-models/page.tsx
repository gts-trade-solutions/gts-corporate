...
/**
 * The schedule is editable from /admin, so this page is revalidated whenever
 * a model is saved. The interval is a backstop for a missed revalidation.
 */
...
const coverageOf = (models: VehicleModel[], oems: string[], markets: string[]) => [
  { value: models.length, label: "Models in the schedule" },
  { value: oems.length, label: "OEMs covered" },
  { value: markets.length, label: "Destination markets" },
  {
    value: new Set(models.flatMap((item) => item.parts)).size,
...
export default async function VehicleModelsPage() {
  ...
  const coverage = coverageOf(vehicleModels, vehicleModelOems, marketsOf(vehicleModels));
  ...
  return (
    <>
      <SplitHero
        ...
        eyebrow="Indian Vehicle Models Gallery"
        lead={`Every model we are regularly asked to supply parts for — ${vehicleModels.length} models across ${vehicleModelOems.length} OEMs — with the components that move most on each one. Pick your model, tick the parts, and the selection carries into the enquiry form.`}
        services={vehicleModelsServiceList}
        cta={{ label: "Browse all models", href: "#models" }}
        secondary={{ label: "Request a part not listed", href: "/contact?enquiry=component-sourcing#rfq" }}
      />
