...
import { findVehicleModelBySlug, listVehicleModels } from "@/lib/vehicle-models-store";

/**
 * Models are editable from /admin, so a page is revalidated whenever its
 * model is saved. The interval is a backstop for a missed revalidation.
 */
export const revalidate = 300;

/** One pre-rendered page per model — long-tail "<model> spare parts" queries. */
export async function generateStaticParams() {
  return (await listVehicleModels()).map((item) => ({ slug: item.slug }));
}

type Props = { params: Promise<{ slug: string }> };

/*
  A result listing cuts the title at roughly 60 characters and the
  description at roughly 160, so both are built to fit rather than written
  long and truncated by the engine. The model name leads either way — it is
  what was searched for, and it is what survives a trim.
*/
const DESCRIPTION_MAX = 158;

const seoTitleFor = (name: string) => `${name} Spare Parts | GTS Trade Solutions`;

const descriptionFor = (item: VehicleModel) => {
  const tail = " Tick the parts you need and enquire in one step.";
  const name = modelName(item);

  /* The engine is the strongest single line of the specification, but it
     only earns its place while part names still fit after it. */
  const withEngine = item.specs?.engine ? `${name} spare parts — ${item.specs.engine}.` : "";
  const head =
    withEngine && DESCRIPTION_MAX - withEngine.length - tail.length > 26
      ? withEngine
      : `${name} spare parts.`;

  // Room left for the parts list, allowing for a space and a full stop.
  const room = DESCRIPTION_MAX - head.length - tail.length - 2;

  const listed: string[] = [];
  for (const part of item.parts) {
    const candidate = [...listed, part.toLowerCase()].join(", ");
    if (candidate.length > room) break;
    listed.push(part.toLowerCase());
  }

  if (!listed.length) return `${head}${tail}`;

  // The list opens a sentence, so it takes a capital even though the part
  // names are lower-cased to read as prose.
  const list = listed.join(", ");
  return `${head} ${list[0]!.toUpperCase()}${list.slice(1)}.${tail}`;
};

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const item = await findVehicleModelBySlug(slug);
  if (!item) return {};
...
export default async function VehicleModelPage({ params }: Props) {
  ...
  const item = vehicleModels.find((entry) => entry.slug === slug);
  if (!item) notFound();
  ...
  /* The technical specification, in the order the client's sheet lists it.
     Fields they have not confirmed yet are simply left out. */
  const specSheet = vehicleSpecFields
    .map((field) => ({ label: field.label, value: item.specs?.[field.key] ?? "" }))
